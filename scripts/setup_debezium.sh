...
check_config_file() {
    ...
    # Check if MSSQL_PASSWORD is set in env
    if [ -f "./config/.env" ]; then
        export $(grep -E '^MSSQL_PASSWORD=' ./config/.env | xargs)
    fi
    if [ -z "$MSSQL_PASSWORD" ]; then
        echo -e "${RED}✗ Please set MSSQL_PASSWORD in ./config/.env${NC}"
        return 1
    fi
...
test_mssql_connectivity() {
    ...
    # Try to connect to MSSQL port using bash /dev/tcp since ping/nc might be missing
    echo "Testing connection to host.docker.internal:1433..."
    
    if docker exec kafka-connect bash -c "timeout 3 bash -c '</dev/tcp/host.docker.internal/1433'" >/dev/null 2>&1; then
        echo -e "${GREEN}✓ successfully connected to host.docker.internal:1433${NC}"
        return 0
    else
...
create_connector() {
    ...
    # Create temporary config with password injected (using Python instead of jq)
    TEMP_CONFIG=$(mktemp)
    # Python one-liner to securely inject the password into the JSON
    python -c "
import json, sys, os
with open(sys.argv[1], 'r') as f:
    data = json.load(f)
data['config']['database.password'] = os.environ.get('MSSQL_PASSWORD', '')
with open(sys.argv[2], 'w') as f:
    json.dump(data, f)
" "$CONNECTOR_CONFIG_FILE" "$TEMP_CONFIG"
    ...
    # Use python instead of jq to parse the response body, or just echo it if not valid JSON
...
check_connector_status() {
    ...
    state=$(echo "$response" | python -c "import sys, json; print(json.load(sys.stdin).get('connector', {}).get('state', 'UNKNOWN'))" 2>/dev/null)
    ...
    if [ "$state" = "RUNNING" ]; then
        ...
        # Check task status
        task_state=$(echo "$response" | python -c "import sys, json; data=json.load(sys.stdin); print(data.get('tasks', [{}])[0].get('state', 'UNKNOWN'))" 2>/dev/null)
...
main() {
    ...
    echo "  curl http://localhost:8083/connectors/$CONNECTOR_NAME/status | python -m json.tool"
    echo ""
